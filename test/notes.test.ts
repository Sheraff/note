...
import { createFolder, moveFile, refreshWorkspace, renameEntry, saveCurrentNote, type NoteConflict, type NoteContext } from '../web/app/notes.ts'
...
describe('moveFile', () => {
  ...
  it('treats moving a file to its current folder as a no-op', async () => {
    ...
    const result = await moveFile(createMockContext(storage, () => {}), 'notes/today.md', 'notes')
