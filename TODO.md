...
- [ ] support other text file types besides `.md`. Keep `.md` as the default for "new note" when the user does not type an extension, but stop force-appending `.md` if they explicitly type `.txt`, `.json`, `.ts`, etc. The current `.md` assumptions live in `web/notes/paths.ts` (`ensureMarkdownExtension`), `web/app/notes.ts` (`createEntry` / note naming), `web/app/FileTree.tsx` (`untitled.md`), and `web/editor/monaco.ts` (hardcoded `markdown` language). Monaco should infer the language from the file path / extension instead of always using markdown.
- [ ] support images. This needs a real file model instead of the current text-only one: both storage backends (`web/storage/file-system-access.ts`, `web/storage/opfs.ts`) and sync (`web/storage/types.ts`, `web/notes/sync.ts`, `server/schemas.ts`, `server/db.ts`, `server/sync.ts`) currently assume every file is UTF-8 text with `content: string`. We need to be able to list, sync, and reopen image files without corrupting them, and show an image preview in the editor pane when the selected file is an image. Also, when a markdown file references an image, we should render that image inline inside Monaco rather than only showing the raw markdown link syntax; Monaco `view zones` look like the right primitive for that, whereas overlay widgets would only float on top of the editor.
- [ ] support arbitrary files (for example PDFs) even when we do not render them inside Monaco. These files should still appear in the file tree, survive storage transfers, and sync correctly between local storage and the server. The likely shape is: text files remain editable, images get a preview mode, and everything else gets a non-editable attachment view in the editor pane with filename / type / size and actions like download or open externally. Conflict handling also needs to become file-type-aware: keep the existing diff flow for text files, but non-text files will need action-based conflict resolution instead of Monaco diff.
